...
namespace TestHL7Enumerator
{
    public class TestDataTypes
    {
        ...
        private const string OBX1 = @"OBX|1|ED|ClinicalPDFReport1^Clinical PDF Report MR077065T-1^^ClinicalPDFReport1^" +
          @"Clinical PDF Report MR077065T-1||MIA^Image^PDF^Base64^JVBERi0xLjQKJeLjz9MKMyAwIG9iaiA8PC9MZW5n\X0D\\X0A\" +
        ...
        public void ShouldConvertIdentifier()
        {
            ...
            Assert.Equal("IDCODE", hl7Id.CheckDigitScheme);
            Assert.Equal("AUSIC", hl7Id.AssigningAuthority.NamespaceId);
            Assert.Equal("NI", hl7Id.IdentifierTypeCode);
            Assert.Equal("RMH", hl7Id.AssigningFacility.NamespaceId);
        ...
        public void ShouldConvertED()
        {
            ...
            var message = new HL7Message(txt);
            var ThirdOBXResult = message.Element("OBX[1].5");
            var hl7Ed = ThirdOBXResult.AsED();
            //MIA^Image^PDF^Base64
            Assert.Equal("MIA", hl7Ed.SourceApplication.NamespaceId);
            Assert.Equal("Image", hl7Ed.TypeOfData);
            Assert.Equal("PDF", hl7Ed.DataSybType);
            Assert.Equal("Base64", hl7Ed.Encoding);
        ...
        public void ShouldConvertFromARepeatingXCNElement()
        {
            ...
            Assert.Equal("AUSHICPR", xcn1.AssigningAuthority.NamespaceId);
            Assert.Equal("L", xcn1.NameTypeCode);
